...
const toLastNames = people => {
  // Solution code here...
  let peopleObject = people.map((obj) =>{
...
const validateEmail = (email) => {
  // Solution code here...
  const emailPattern = new RegExp(/^[^\.][a-zA-Z0-9]*(?!\.@)(?:(\.)[a-zA-Z0-9]*@|@)[a-zA-Z0-9]*\.(net|com|org)$/);
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const numPattern = new RegExp(/^(\d{3}|[(]\d{3}[)])(\s|[-])?\d{3}(\s|[-])?\d{4}$/);
